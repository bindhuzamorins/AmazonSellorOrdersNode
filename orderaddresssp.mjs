...
export const handler = async (event) => {
  try {
    ...
    const apiUrl = `https://sandbox.sellingpartnerapi-na.amazon.com/orders/v0/orders/${AmOrderId}/address`;
    ...
    const request = {
      ...
      path: `/orders/v0/orders/${AmOrderId}/address`,
    };
    ...
    const address = orderData?.payload?.ShippingAddress;
    if (!address) {
      throw new Error('Shipping address is not available in the response.');
    }

    const { Name, AddressLine1, AddressLine2, AddressLine3,City,Country,District, StateOrRegion, PostalCode, CountryCode,Phone,AddressType } = address;

    // Call the stored procedure
    const procedureCall = `CALL sp_add_update_order_address(?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,@rcode,@message)`;
    const values = [
        client_code,
        order_id,
        AmOrderId,
        orderData?.payload?.BuyerCompanyName ||'',
      Name,
      AddressLine1,
      AddressLine2 || '',
      AddressLine3 || '',
